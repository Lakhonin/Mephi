...
-- 5. название издания что больше(по заявкам) заказывают + автор издания (одним полем)
...
case when c.count_publication is not null then c.count_publication else '0' end 'Кол-во издания на складе'
from Кафедра_E7 e7 
...
left join (select b.id_chair,c.id_publication,d.title,d.name,d.surname,d.middlename,d.count_publication from (select max(a.id_count_publication) max_id_count_publication, a.id_chair from
...
left join (select count(e14.id_publication) max_id_count_publication, e8.id_chair, id_publication from  Заявка_E8 e8 left join Заявки_в_заказе_E14 e14 on e14.id_task = e8.id_task group by e8.id_chair,e14.id_publication)
